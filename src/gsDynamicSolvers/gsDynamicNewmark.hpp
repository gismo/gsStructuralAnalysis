...
gsDynamicImplicitEuler<T>::_step_impl(const T dt)
{
  ...
  gsSparseMatrix<> lhs = K + 4/(math::pow(gamma*m_dt,2))*M + 2/(gamma*m_dt)*C;
  gsMatrix<> rhs = F + M*(4/(math::pow(gamma*m_dt,2))*(m_Uold)+4/(gamma*m_dt)*m_Vold+m_Aold) + C*(2/(gamma*m_dt)*(m_Uold)+m_Vold);

  m_solver.compute(lhs);
  m_uNew = solver.solve(rhs);

  m_vNew = 2/(gamma*m_dt)*(m_uNew-m_Uold) - m_Vold;
  m_aNew = (m_uNew-m_Uold-m_Vold*m_dt)*4/(math::pow(gamma*m_dt,2)) - m_Aold;
  m_time += m_dt;
