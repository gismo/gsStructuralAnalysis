...
int main(int argc, char *argv[])
{
    ...
    Jacobian = [&geom,&bb2,&assembler](gsVector<real_t> const &x, gsSparseMatrix<real_t> & m)
    {
    ...
    Residual = [&geom,&bb2,&assembler](gsVector<real_t> const &x, gsVector<real_t> & result)
    {
        ThinShellAssemblerStatus status;
        gsMatrix<real_t> solFull = assembler.fullSolutionVector(x);
        GISMO_ASSERT(solFull.rows() % 3==0,"Rows of the solution vector does not match the number of control points");
        ...
        status = assembler.assembleVector(def,false);
        result = assembler.rhs();
        return status == ThinShellAssemblerStatus::Success;
    };

    // Function for the ALResidual
    ALResidual = [&DY,&geom,&bb2,&assembler](gsVector<real_t> const &x, real_t lam, gsVector<real_t> & result)
    {
        ThinShellAssemblerStatus status;
        DY.setValue(lam,3);
        gsMatrix<real_t> solFull = assembler.fullSolutionVector(x);
