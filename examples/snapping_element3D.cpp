...
int main(int argc, char *argv[])
{
    ...
    gsMultiPatch<> mp;
    for (size_t p=0; p!=element.nPatches(); p++)
    {
        gsTensorBSpline<3,real_t>::uPtr tmp = gsNurbsCreator<>::lift3D(dynamic_cast<gsTensorBSpline<2,real_t> &>(element.patch(p)),3e-3);
        mp.addPatch(*tmp,element.patch(p).label());
    }
    ...
    bc.addCondition(6,boundary::east,condition_type::dirichlet,0,2);

    bc.addCondition(8,boundary::west,condition_type::dirichlet,0,0);
    bc.addCondition(8,boundary::west,condition_type::dirichlet,0,2);

    bc.addCondition(12,boundary::east,condition_type::dirichlet,0,0);
    bc.addCondition(12,boundary::east,condition_type::dirichlet,0,2);
    ...
    bc.addCondition(13,boundary::north,condition_type::dirichlet,0,2);
    ...
    bc.addCondition(14,boundary::north,condition_type::dirichlet,0,2);
