...
//------------------------------------------------------------------------------
    ...
    assembler->setGoal(GoalFunction::Displacement,2);
...
//------------------------------------------------------------------------------
    ...
    uOld = assembler->projectL2_L(initialShape);
    // Set initial velocity to zero
    vOld.setZero();
    aOld = assembler->projectL2_L(initialAcc);

    // Compute Error
    gsMultiPatch<> deformationIni = mp;
    assembler->constructSolutionL(uOld,deformationIni);
...
for (index_t i=0; i<steps; i++)
{
  ...
  assembler->constructSolutionL(uNew,mp_def);
  displacements = assembler->constructDisplacementL(uNew);
  gsField<> solField(mp,displacements);
  ...
  gsVector<> solVector;
  gsMultiPatch<> uL, aL,dualL,dualH;
  gsSparseSolver<>::LU solver;
  gsSparseMatrix<> M_L, M_H;

  assembler->constructMultiPatchL(uNew,uL);
  assembler->constructMultiPatchL(aNew,aL);
  ...
  assembler->assembleDualL(uL); // displacement-based goal function
  gsInfo << "done.\n";

  gsInfo << "Solving dual (L), size = "<<assembler->matrixL().rows()<<","<<assembler->matrixL().cols()<<"... "<< std::flush;
  solver.compute(assembler->massL() + assembler->matrixL());
  ...
  assembler->assembleMassH();
  gsInfo << "done.\n";

  gsInfo << "Assembling dual vector (H)... "<< std::flush;
  assembler->assembleDualH(uL); // displacement-based goal function
  gsInfo << "done.\n";

  gsInfo << "Solving dual (H), size = "<<assembler->matrixH().rows()<<","<<assembler->matrixH().cols()<<"... "<< std::flush;
  solver.compute(assembler->massH() + assembler->matrixH());
  solVector = solver.solve(assembler->dualH());
  assembler->constructMultiPatchH(solVector,dualH);
  gsInfo << "done.\n";


  real_t Ei = (assembler->computeErrorInertia(dualL,dualH,aL));
  real_t Es = (assembler->computeError(dualL,dualH,mp_def));
  gsInfo<<"Error (stiffness) = "<<Es<<"\n";
  gsInfo<<"Error (inertia  ) = "<<Ei<<"\n";

  gsInfo<<"Ei-Es = "<<Ei-Es<<"\n";
  gsInfo<<"Ei+Es = "<<Ei+Es<<"\n";

  gsInfo<<"Goal = "<<(assembler->computeGoal(mp_def))<<"\n";

  typedef gsExprAssembler<>::geometryMap geometryMap;

  gsExprAssembler<> A(1,1);
  gsExprEvaluator<> ev(A);
  gsFunctionExpr<> anFun(w_an,3);
  gsFunctionExpr<> uv3("0","0","1",3);
  geometryMap Gori = A.getMap(mp);
  auto fun1d = A.getCoeff(anFun,Gori);
  auto fun3d = A.getCoeff(analytical,Gori);
  auto sol = A.getCoeff(displacements);
  auto uv = A.getCoeff(uv3,Gori);

  A.setIntegrationElements(basisL);
  gsVector<> pt(2);
  pt.setConstant(0.25);
  gsInfo<<(ev.integral((fun3d-sol).tr() * uv * meas(Gori)))<<"\n";
  ...
  if (write)
  {
    ...
    v<<  0.0,0.0;
    gsMatrix<> res2;
    displacements.patch(0).eval_into(v,res2);
    std::ofstream file;
  ...
  if (i==steps-1)
  {
    ...
    gsInfo <<"Maximum deformation coef: "
           << displacements.patch(0).coefs().colwise().maxCoeff() <<".\n";
    gsInfo <<"Minimum deformation coef: "
           << displacements.patch(0).coefs().colwise().minCoeff() <<".\n";
  }
