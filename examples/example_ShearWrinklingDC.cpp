...
int main (int argc, char** argv)
{
    ...
    if(membrane)
        assembler = new gsThinShellAssemblerDWR<3, real_t, false>(mp,basisL,basisH,BCs,force,materialMatrix);
    else
    ...
    Residual_t Residual = [&assembler,&mp_def](gsVector<real_t> const &x)
    {
        ...
        return assembler->primalL(); // - lam * force;
    };

    assembler->assembleL();
    gsSparseMatrix<> matrix = assembler->matrixL();
    gsVector<> vector = assembler->primalL();
    ...
    index_t m_maxIterations = 20;

    real_t m_relax = 1.0;
    real_t m_toleranceU = tolU;
    real_t m_toleranceF = tolF;

    gsSparseSolver<>::CGDiagonal m_solver;

    gsSparseMatrix<> m_jacMat;
    ...
    while (D-dL < Dtarget.back())
    {
      ...
      m_converged = false;
      gsVector<> DeltaU = gsVector<>::Zero(solVector.rows());
      gsVector<> deltaU = gsVector<>::Zero(solVector.rows());
      if (solVector.rows()==0)
      {
        assembler->assembleL();
        matrix = assembler->matrixL();
        vector = assembler->primalL();
        // assembler->assemble();
        ...
        deltaU = DeltaU = m_solver.solve(vector);
        gsDebugVar(deltaU);
        gsDebugVar(DeltaU);
        solVector.resize(deltaU.rows(),1);
        solVector.setZero();
      }

      gsVector<> resVec = Residual(solVector+DeltaU);
      real_t residual = resVec.norm();
      ...
      for (m_iterations = 0; m_iterations != m_maxIterations; ++m_iterations)
      {
          ...
          gsInfo<<std::setw(17)<<std::left<<m_relax * deltaU.norm()/(solVector+DeltaU).norm();
          ...
          if (m_relax * deltaU.norm()/(solVector+DeltaU).norm()  < m_toleranceU && residual/residual0 < m_toleranceF)
          {
              m_converged = true;
              solVector+=DeltaU;
              break;
      ...
      if (stress)
      {
        ...
        assembler->constructStress(mp_def,stresses,stress_type::principal_stress_membrane);
...
template <class T>
gsMultiPatch<> RectangularDomain(int n, int p, T L, T B, bool clamped, T clampoffset)
{
  gsMultiPatch<> mp = RectangularDomain(n, n, p, p, L, B, clamped, clampoffset);
  return mp;
}

template <class T>
gsMultiPatch<> RectangularDomain(int n, int m, int p, int q, T L, T B, bool clamped, T clampoffset)
{
  // -------------------------------------------------------------------------
  // --------------------------Make beam geometry-----------------------------
  // -------------------------------------------------------------------------
...
template <class T>
gsMultiPatch<> Rectangle(T L, T B) //, int n, int m, std::vector<boxSide> sides, T offset)
{
  // -------------------------------------------------------------------------
  // --------------------------Make beam geometry-----------------------------
  // -------------------------------------------------------------------------
...
template <class T>
gsMultiPatch<> AnnularDomain(int n, int p, T R1, T R2)
{
  // -------------------------------------------------------------------------
  // --------------------------Make beam geometry-----------------------------
  // -------------------------------------------------------------------------
...
template <class T>
gsMultiPatch<> FrustrumDomain(int n, int p, T R1, T R2, T h)
{
  // -------------------------------------------------------------------------
  // --------------------------Make beam geometry-----------------------------
  // -------------------------------------------------------------------------
...
{
  gsMatrix<> P(2,1), Q(2,1);
  gsMatrix<> out(3,points.cols());
  gsMatrix<> tmp;
  ...
  {
    gsMatrix<> out2(kmax,points.cols()); // evaluation points in the rows, output (per coordinate) in columns
...
void writeSectionOutput(const gsMultiPatch<T> & mp, const std::string dirname, const index_t coordinate, const T coordVal, const index_t N, bool undeformed) // coordinate: the column which remains constant at coordVal
{
  ...
  gsMatrix<> out(3,N); // evaluation points in the rows, output (per coordinate) in columns
    for (int k = 0; k != N; k ++)
    {
