...
int main (int argc, char** argv)
{
    ...
    B = 0.14; L = 2*B; // total length and width
    pratio = 1/4.;
    Bp= B*pratio;
    Lp= L*pratio;
    ...
    /*
              ...
              |  2   |            3            |    |
              |      |                         |    |
              |______|_________________________|    | B/2
          |   |      |                         |    |
    Bp/2  |   |  0   |            1            |    |
    ...
    for (size_t p=0; p!=4; ++p)
      mp.addPatch(gsNurbsCreator<real_t>::BSplineSquare());

    mp.patch(0).coefs().col(0).array() *= Lp/2.;
    mp.patch(0).coefs().col(1).array() *= Bp/2.;

    mp.patch(1).coefs().col(0).array() *= (L-Lp)/2.;
    mp.patch(1).coefs().col(1).array() *= Bp/2.;
    mp.patch(1).coefs().col(0).array() += Lp/2.;

    mp.patch(2).coefs().col(0).array() *= Lp/2.;
    mp.patch(2).coefs().col(1).array() *= (B-Bp)/2.;
    mp.patch(2).coefs().col(1).array() += Bp/2.;

    mp.patch(3).coefs().col(0).array() *= (L-Lp)/2.;
    mp.patch(3).coefs().col(1).array() *= (B-Bp)/2.;
    mp.patch(3).coefs().col(0).array() += Lp/2.;
    mp.patch(3).coefs().col(1).array() += Bp/2.;
    ...
    gsInfo<<"alpha = "<<L/B<<"; beta = "<<B/thickness<<"\n";

    gsMultiBasis<> dbasis(mp);
    for(size_t p=0; p!=mp.nPatches(); p++)
    {
      gsInfo<<"Basis (patch "<<p<<"): "<< mp.patch(p).basis() << "\n";
    }

    // for (size_t p = 0; p!=mp.nPatches(); p++)
    //   gsInfo<<"Patch "<<std::to_string(p)<<": "<<mp.patch(p).coefs()<<"\n";
    ...
    BCs.addCondition(2,boundary::west, condition_type::dirichlet, 0, 0 ,false,0);

    // Right side of the sheet
    BCs.addCondition(1,boundary::east, condition_type::collapsed, 0, 0 ,false,0);
    BCs.addCondition(1,boundary::east, condition_type::dirichlet, 0, 0 ,false,1);
    BCs.addCondition(1,boundary::east, condition_type::dirichlet, 0, 0 ,false,2);
    ...
    BCs.addCondition(2,boundary::west, condition_type::clamped  , 0, 0, false,2);
    // Clamp Right
    ...
    // Symmetry on south of the sheet
    BCs.addCondition(0,boundary::south, condition_type::dirichlet, 0, 0, false, 1 );
    BCs.addCondition(0,boundary::south, condition_type::dirichlet, 0, 0, false, 2 );

    BCs.addCondition(1,boundary::south, condition_type::dirichlet, 0, 0, false, 1 );
    BCs.addCondition(1,boundary::south, condition_type::dirichlet, 0, 0, false, 2 );
    ...
    dirname = dirname + "/QuarterSheet_-r" + std::to_string(numHref) + "-e" + std::to_string(numElevate) + "-M" + std::to_string(material) + "-c" + std::to_string(Compressibility) + "-alpha" + std::to_string(L/B) + "-beta" + std::to_string(B/thickness);
    ...
    // assembler->options().setReal("IfcDirichlet",1e8);
    // assembler->options().setReal("IfcClamped",1e6);
    ...
    for (index_t k=0; k<step; k++)
    {
      ...
      if (plot)
      {
        ...
        for (size_t p=0; p!=mp.nPatches(); p++)
        {
          fileName = output + util::to_string(k) + std::to_string(p);
