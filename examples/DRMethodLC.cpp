...
public:
    ...
    m_residualFun(Residual)
    {
        m_dt = 1.0;
        m_dofs = m_stif.rows();
        m_v = m_U = m_DeltaU = m_du = gsVector<T>(m_dofs);
        m_v.setZero();
        m_U.setZero();
        m_DeltaU.setZero();
        m_du.setZero();
    ...
    void iteration()
    {
        ...
        m_R = m_residualFun(m_U+m_DeltaU,m_L+m_DeltaL,m_force) - m_damp*m_v;
//----------------------------------------------------------------------------------
        m_v += m_dt * m_massInv * m_R;                    // Velocities at t+dt/2
        m_du = m_dt * m_v;               // Velocities at t+dt
//----------------------------------------------------------------------------------
        // m_du = m_du + m_dt*m_dt*m_massInv*m_R;
        // m_v = m_du/m_dt;
//----------------------------------------------------------------------------------

        m_DeltaU += m_du;               // Velocities at t+dt
        m_Ek = m_v.transpose() * m_mass * m_v;
    }
    ...
    void step(T loadStep)
    {
        ...
        for (index_t k=0; k!=m_maxIt; k++)
        {
            ...
            gsInfo  <<std::setw(4)<<k
                    <<std::setw(16)<<m_R.norm()/m_force.norm()
                    <<std::setw(16)<<m_Ek/EkIni
                    <<std::setw(16)<<(m_U+m_DeltaU+m_du).norm()
                    <<std::setw(16)<<m_DeltaU.norm()
                    <<std::setw(16)<<m_du.norm()
                    <<std::setw(16)<<m_L+m_DeltaL
                    <<std::setw(16)<<m_DeltaL
                    <<std::setw(16)<<m_DeltaU.norm()+m_DeltaL*m_DeltaL
                    <<"\n";

            if (m_R.norm()/m_force.norm() < m_tolF && m_Ek/EkIni < m_tolE)
            {
                m_L += m_DeltaL;
                m_U += m_DeltaU;
                break;
    ...
    {
        m_R = m_residualFun(m_U+m_DeltaU,m_L+m_DeltaL,m_force)- m_damp*m_v;
        m_du = - 1.5 * m_dt * m_v + m_dt*m_dt / 2. * m_massInv * m_R;
        m_DeltaU += m_du;
...
//----------------------------------------------------------------------------------
        m_R = m_residualFun(m_U+m_DeltaU,m_L+m_DeltaL,m_force);
        m_du = 0.5*m_dt*m_dt*m_massInv*m_R;
        m_v = m_du/m_dt;
//----------------------------------------------------------------------------------
        // m_R = m_residualFun(m_U+m_DeltaU,load,m_force)- m_damp*m_v;
        // m_v = 0.5 * m_dt * m_massInv * m_R; // Velocities at dt/2
        // m_du = m_dt * m_v;               // Velocities at t+dt
//----------------------------------------------------------------------------------
