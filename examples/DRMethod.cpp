...
template <class T>
class gsDynamicRelaxation
{
public:
    /// Constructor given the matrices and timestep
    gsDynamicRelaxation(    const gsSparseMatrix<T> & K,
    ...
    {
        m_Ek_prev = m_Ek;
        m_R = m_residualFun(m_u) - m_damp*m_v;
        m_v += m_dt * m_massInv * m_R;                    // Velocities at t+dt/2
        m_u += m_dt * m_v;               // Velocities at t+dt
        m_Ek = m_v.transpose() * m_mass * m_v;
    }
    ...
    {
        m_R = m_residualFun(m_u)- m_damp*m_v;
        m_u += - 1.5 * m_dt * m_v + m_dt*m_dt / 2. * m_massInv * m_R;
...
protected:
    ...
    const std::function < gsVector<T> ( gsVector<T> const & ) > m_residualFun;
...
int main(int argc, char *argv[])
{
    ...
    gsDynamicRelaxation<real_t> DRM(K,Residual);
